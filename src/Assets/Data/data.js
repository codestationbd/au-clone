const data = {
    blueTiles: [
        {
            title: "My Asian",
            body: "Online Student Portal",
            img: "",
        },
        {
            title: "Programs",
            body: "AUB offers a wide range of Programs.",
            img: "icon-education",
        },
    ],

    msgs: [
        {
            img: "founder.jpg",
            title: "MESSAGE FROM THE HONOURABLE FOUNDER & FOUNDER VICE CHANCELLOR",
            designation: "Professor Abulhasan M. Sadeq, Ph.D.",
            body: "It gives me great pleasure to welcome you to Asian University of Bangladesh, a place of learning and scholarship, engaged in preparing young men and women to face the challenges of the new millennium.In its efforts towards progress and development, Bangladesh needs high quality human resources, professionally sound and morally upright, equipped with the latest knowledge in multi-disciplinary fields and with technological ability. AUB commits to deliver this to the nation.",
        },
        {
            img: "vc.jpg",
            title: "VICE CHANCELLOR | SHORT BIOGRAPHY OF PROFESSOR SHAHJAHAN KHAN, PHD",
            designation:
                "Professor Dr Shahjahan Khan is the Vice Chancellor and Chief Executive Officer of the Asian University of Bangladesh.",
            body: "Professor Shahjahan Khan obtained his PhD and MSc degrees in Mathematical Statistics from the University of Western Ontario (UWO), Canada; and BSc (Hon) and MSc in Statistics from Jahangirnagar University, Bangladesh. He started his academic career as a Lecturer at Dhaka University, Bangladesh in 1980, and was awarded Commonwealth Scholarship in the UK. He joined the University of Southern Queensland (USQ), Australia in 1993 as a Lecturer of Statistics. He served as the founding Professor of Statistics at USQ. In addition to teaching at UWO, Canada he has taught at University of Dhaka, Bangladesh; King Fahd University of Petroleum and Minerals, Saudi Arabia; Sultan Qaboos University, Oman; and University of Bahrain, Bahrain. At USQ, he has served as the Statistics Discipline leader, member on the Academic Senate/Board and Chair of Multicultural Staff Network.",
        },
    ],

    navTree: [
        {
            title: "Home",
        },
        {
            title: "About AUB",
            submenu: [
                {
                    title: "Aub Details",
                    submenu: [
                        {
                            title: "Message From Vc",
                        },
                        {
                            title: "Background",
                        },
                        {
                            title: "Philoshopy and mission",
                        },
                    ],
                },
                {
                    title: "Administration",
                    submenu: [
                        {
                            title: "board of trustees",
                        },
                        {
                            title: "syndicate members",
                        },
                        {
                            title: "deans & heads",
                        },
                        {
                            title: "management",
                        },
                    ],
                },
                {
                    title: "Faculty",
                },
            ],
        },
        {
            title: "Schools & Departments",
            submenu: [
                {
                    title: "THE SCHOOL OF SCIENCE AND eNGINEERING",
                    submenu: [
                        {
                            title: "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING",
                        },
                    ],
                },
                {
                    title: "THE SCHOOL OF SOCIAL SCIENCE",
                    submenu: [
                        {
                            title: "DEPARTMENT OF SOCIOLOGY AND ANTHROPOLOGY",
                        },
                        {
                            title: "DEPARTMENT OF SOCIAL WORK",
                        },
                        {
                            title: "DEPARTMENT OF ECONOMICS",
                        },
                        {
                            title: "DEPARTMENT OF INFORMATION SCIENCE AND LIBRARY MANAGEMENT",
                        },
                        {
                            title: "DEPARTMENT OF GOVERMENT AND POLITICS",
                        },
                    ],
                },
                {
                    title: "THE SCHOOL OF ARTS",
                    submenu: [
                        {
                            title: "DEPARTMENT OF ENGLISH",
                        },
                        {
                            title: "DEPARTMENT OF BENGALI",
                        },
                        {
                            title: "DEPARTMENT OF ISLAMIC STUDIES",
                        },
                        {
                            title: "DEPARTMENT OF ISLAMIC HISTORY AND CIVILIZATION",
                        },
                    ],
                },
                {
                    title: "THE SCHOOL OF EDUCATION AND TRAINING",
                    submenu: [
                        {
                            title: "DEPARTMENT OF EDUCATION AND TRAINING",
                        },
                    ],
                },
                {
                    title: "THE SCHOOL OF BUSINESS AND ADMINISTRATION",
                    submenu: [
                        {
                            title: "DEPARTMENT OF BUSINESS AND ADMINISTRATION",
                        },
                    ],
                },
            ],
        },
        {
            title: "Programs",
            submenu: [
                {
                    title: "BACHELOR OF BUSINESS ADMINISTRATION (BBA)",
                },
                {
                    title: "B.SC.(ENGG) IN COMPUTER SCIENCE AND ENGINEERING (CSE)",
                },
                {
                    title: "B.S.S.(HONS.) IN GOVERMENT AND POLITICS",
                },
                {
                    title: "B.S.S.(HONS) SOCIOLOGY AND ANTHROPOLOGY",
                },
                {
                    title: "B.S.S.(HONS) IN ECONOMICS",
                },
                {
                    title: "B.A.(HONS) IN ENGLISH",
                },
                {
                    title: "B.A.(HONS) IN BENGALI",
                },
                {
                    title: "B.A.(HONS) IN ISLAMIC STUDIES",
                },
                {
                    title: "B.A.(HONS) IN ISLAMIC HISTORY AND CIVILIZATION",
                },
                {
                    title: "BACHELOR OF EDUCATION (B.ED)",
                },
                {
                    title: "B.S.S. (HONS) IN SOCIAL WORK",
                },
                {
                    title: "MASTER OF BUSINESS AND ADMINISTRATION",
                },
                {
                    title: "M.A. IN BENGALI",
                },
                {
                    title: "M.A. IN ENGLISH",
                },
                {
                    title: "M.A. IN ISLAMIC HISTORY AND CIVILIZATION",
                },
                {
                    title: "M.A. IN ISLAMIC STUDIES",
                },
                {
                    title: "M.S.S. IN ECONOMICS",
                },
                {
                    title: "M.S.S. IN GOVERNMENTS AND POLITICS",
                },
                {
                    title: "M.S.S. IN SOCIOLOGY AND ANTHROPOLOGY",
                },
                {
                    title: "M.S.S. IN SOCIAL WORK",
                },
                {
                    title: "M.S.S. IN INFORMATION SCIENCE AND LIBRARY MANAGEMENT",
                },
                {
                    title: "MASTER OF EDUCATION(M.ED)",
                },
            ],
        },
        {
            title: "IQAC",
        },
        {
            title: "ALUMNI",
        },
        {
            title: "Contact",
        },
    ],

    latest: {
        news: [
            {
                img: "AubBennar1_15August.jpg",
                title: "National Mourning Day 2020 at AUB",
                date: "2020-08-15"
            },
            {
                img: "SeminarNewZealand.jpg",
                title: "Career Seminar on 'Higher Study in New Zealand' held at AUB's permanent Campus, Ashulia.",
                date: "2020-01-29"
            },
            {
                img: "FamilyDay20.jpg",
                title: "AUB Family Day 2020",
                date: "2020-01-28"
            },
            {
                img: "Seminar_AUS.jpg",
                title: "Seminar on 'Universal Values for Peaceful Co-existence in Culturally Diverse World'",
                date: "2020-08-15"
            },
            {
                img: "AnniversAUB24th.jpg",
                title: "24th University Day 2020",
                date: "2020-01-04"
            },
            {
                img: "victory_day_pic.jpg",
                title: "Victory Day (2019) celebrated at AUB",
                date: "2019-12-18"
            }
        ]
    },

    programs_offered: [
        "BACHELOR OF BUSINESS ADMINISTRATION (BBA)",
        "B.SC.(ENGG.) IN COMPUTER SCIENCE AND ENGINEERING (CSE)",
        "B.S.S.(HONS.) IN GOVERNMENT AND POLITICS",
        "B.S.S. (HONS.) IN SOCIOLOGY AND ANTHROPOLOGY",
        "B.S.S.(HONS) IN ECONOMICS",
        "B.A. (HONS) IN ENGLISH",
        "B.A. (HONS) IN BENGALI",
        "BA (HONS) IN ISLAMIC STUDIES",
        "B.A. (HONS) IN ISLAMIC HISTORY AND CIVILIZATION",
        "BACHELOR OF EDUCATION (B.ED)",
        "B.S.S. (HONS) IN SOCIAL WORK",
        "MAster of Business Administraton",
        "M.A. in Bengali",
        "M.A. in English",
        "M.A. in Islamic History and Civilization",
        "M.A in Islamic Studies",
        "M.S.S in Goverment and Politics",
        "M.S.S in Information Science and Library Management",
        "Master of Education (M.Ed)",
    ],
};

export default data;
